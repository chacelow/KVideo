...
export function PopularFeatures({ onSearch, contentType: propContentType }: PopularFeaturesProps) {
  const {
    tags,
    selectedTag,
    contentType,
  ...
  const activeContentType = propContentType || contentType;
  const {
  ...
  const {
    ...
    tags,
    activeContentType === 'anime' ? 'movie' : activeContentType
  );
  ...
  return (
    <div className="animate-fade-in">
      ...
      {activeContentType === 'anime' && !isTagManagementMode && !effectiveRecommendSelected ? (
        <AnimePortal onSearch={(title) => onSearch?.(title)} />
      ) : (
        <>
          <TagManager
            tags={tags}
